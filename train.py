...
class Instructor:
    ...
    def load_and_test(self):
        if self.opt.load_model != "":
            self.model.load_state_dict(torch.load(self.opt.load_model))
        print(f"loading model from {self.opt.load_model}")
        self.model.eval()
        ...
        f1_train = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2], average='macro')
        print(f"train_acc: {train_acc}, train_f1: {f1_train}")
        ...
        test_acc = n_test_correct / n_test_total
        f1_test = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2], average='macro')
        print(f"test_acc: {test_acc}, test_f1: {f1_test}")
